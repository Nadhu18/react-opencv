class CV {
    ...
    _dispatch(event) {
        ...
        // let tempEvent = JSON.parse(JSON.stringify(event));
    ...
    load() {
        this._status = {}
        this.worker = new Worker(`${process.env.PUBLIC_URL}/js/cv.worker.js`) // load worker

        // Capture events and save [status, event] inside the _status object
        this.worker.onmessage = e => { console.log("onmessage success", e); this._status[e.data.msg] = ['done', e] }
        this.worker.onerror = e => { console.log(e); this._status[e.data.msg] = ['error', e] }
    ...
    scanImage(payload) {
        return this._dispatch({ msg: 'scanImage', payload })
    }
}
